# AMSF001 Development Playbook v19
...
2. **Learning System**
   - Classification rules stored per project
   - Learns from user corrections
   - Confidence increases with usage
   - Pattern matching for merchants
...
3. **User Interface**
   ...
   - Step indicator (Upload → Scan → Review)
...
See `RECEIPT-SCANNER-DEPLOYMENT.md` for complete checklist:
1. Run SQL migration in Supabase
2. Create `receipt-scans` storage bucket
3. Add `ANTHROPIC_API_KEY` to Vercel environment
4. Deploy and test

#### Cost Analysis
- **Per Receipt:** ~$0.01 (Claude Haiku)
- **Medium Usage (600/month):** ~$7/month
- **ROI:** 31,000%+ in time savings

#### Integration Point
The scanner is integrated into `Expenses.jsx`:
- "Scan Receipt" button with sparkles icon
- Opens ReceiptScanner component
- Creates expense on completion
- Learns from user corrections
