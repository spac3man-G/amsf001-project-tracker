 ...
 */
export function canEditTimesheet(userRole, timesheet, currentUserId) {
  if (isOneOf(userRole, [ROLES.ADMIN, ROLES.SUPPLIER_PM])) return true;
  
  // Owner can edit if not yet approved
  const isOwner = timesheet?.created_by === currentUserId || 
                  timesheet?.resource?.user_id === currentUserId;
 ...
 */
export function canDeleteTimesheet(userRole, timesheet, currentUserId) {
  if (userRole === ROLES.ADMIN) return true;
  
  const isOwner = timesheet?.created_by === currentUserId || 
                  timesheet?.resource?.user_id === currentUserId;
  return isOwner && timesheet?.status === 'Draft';
}

/**
 * Can the user submit this timesheet for approval?
 */
export function canSubmitTimesheet(userRole, timesheet, currentUserId) {
 ...
 */
export function canValidateExpense(userRole, expense) {
