...
export default function Milestones() {
  ...
  async function handleDelete(id) {
    if (!confirm('Delete this milestone? This may affect linked timesheets and expenses.')) return;
  ...
  const avgCompletion = milestones.length > 0 
    ...
    <div className="page-container">
      ...
      <div className="stats-grid" style={{ marginBottom: '1.5rem' }}>
        ...
        <div className="stat-card">
          ...
          <div className="stat-value" style={{ color: getProgressColor(avgCompletion) }}>{avgCompletion}%</div>
        </div>
      </div>
        ...
        <div className="card" style={{ marginBottom: '1.5rem', border: '2px solid var(--primary)' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem' }}>
            <button className="btn btn-primary" onClick={handleAdd} disabled={saving}>
              <Save size={16} /> {saving ? 'Saving...' : 'Save Milestone'}
            </button>
            ...
            <div key={ms.id} className="card" style={{ position: 'relative' }}>
              {editingId === ms.id ? (
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem' }}>
                    <button className="btn btn-primary" onClick={() => handleSave(ms.id)} disabled={saving}>
                ...
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                    ...
                    <span className={`status-badge ${getStatusColor(ms.status)}`}>
                      {getStatusIcon(ms.status)}
                      <span style={{ marginLeft: '0.25rem' }}>{ms.status}</span>
                    </span>
                  </div>
                  
                  {ms.description && (
                    <p style={{ fontSize: '0.9rem', color: '#64748b', margin: '0 0 0.75rem 0' }}>{ms.description}</p>
                  )}
                  ...
                  <div style={{ display: 'flex', gap: '1rem', fontSize: '0.85rem', color: '#64748b', marginBottom: '0.75rem' }}>
                    {ms.start_date && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                        <Calendar size={14} />
                        Start: {new Date(ms.start_date).toLocaleDateString('en-GB')}
                      </div>
                    )}
                    {ms.due_date && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                        <Calendar size={14} />
                        Due: {new Date(ms.due_date).toLocaleDateString('en-GB')}
                      </div>
                    )}
                  </div>
                  ...
                  <div style={{ marginBottom: '0.75rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', marginBottom: '0.25rem' }}>
                      ...
                      <span style={{ fontWeight: '600', color: getProgressColor(ms.completion_percentage || 0) }}>
                        {ms.completion_percentage || 0}%
                      </span>
                    </div>
                    <div style={{ height: '8px', backgroundColor: '#e2e8f0', borderRadius: '4px', overflow: 'hidden' }}>
                      <div style={{ 
                        width: `${ms.completion_percentage || 0}%`, 
                        height: '100%', 
                        backgroundColor: getProgressColor(ms.completion_percentage || 0),
