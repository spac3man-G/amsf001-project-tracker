...
import { 
  FileText, Download, FileSpreadsheet, Calendar, RefreshCw, TrendingUp, Clock,
  Receipt, Milestone, Users, Award, Filter, ChevronDown, ChevronUp, FileCheck,
  Building2, UserCheck, DollarSign
} from 'lucide-react';
import {
  generateProjectSummaryReport, generateTimesheetReport, generateExpenseReport,
  generateKPIReport, generateCustomerInvoice, generatePartnerInvoice,
  exportTimesheetsCSV, exportExpensesCSV
} from '../utils/pdfGenerator';
import {
  canGenerateCustomerInvoice, canGenerateThirdPartyInvoice, canSeeCostPrice
...
export default function Reports() {
  ...
  const [stats, setStats] = useState({
    totalHours: 0, totalExpenses: 0, approvedTimesheets: 0,
    pendingTimesheets: 0, approvedExpenses: 0, pendingExpenses: 0
  });
  ...
  useEffect(() => { calculateStats(); }, [timesheets, expenses]);
  ...
  function getThirdPartyPartners() { return resources.filter(r => r.resource_type === 'third_party'); }
  function getCompletedMilestones() { return milestones.filter(m => m.status === 'Completed' || m.percent_complete >= 100); }
    ...
    <div style={{ padding: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <div>
          ...
          <p style={{ margin: '0.25rem 0 0', color: '#64748b', fontSize: '0.875rem' }}>
            Generate PDF reports, export data, and create invoices
          </p>
        </div>
        <button onClick={fetchAllData} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', backgroundColor: '#f1f5f9', color: '#475569', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '500' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
        <div style={{ backgroundColor: 'white', padding: '1rem', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
          ...
          <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#0f172a' }}>{stats.totalHours}h</div>
          <div style={{ fontSize: '0.75rem', color: '#64748b' }}>{(stats.totalHours / 8).toFixed(1)} days</div>
        </div>
        <div style={{ backgroundColor: 'white', padding: '1rem', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
          ...
          <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#0f172a' }}>{formatCurrency(stats.totalExpenses)}</div>
        </div>
        <div style={{ backgroundColor: 'white', padding: '1rem', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
          ...
          <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#22c55e' }}>{stats.approvedTimesheets + stats.approvedExpenses}</div>
          <div style={{ fontSize: '0.75rem', color: '#64748b' }}>{stats.approvedTimesheets} timesheets, {stats.approvedExpenses} expenses</div>
        </div>
        <div style={{ backgroundColor: 'white', padding: '1rem', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
          ...
          <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#f59e0b' }}>{stats.pendingTimesheets + stats.pendingExpenses}</div>
          <div style={{ fontSize: '0.75rem', color: '#64748b' }}>{stats.pendingTimesheets} timesheets, {stats.pendingExpenses} expenses</div>
        </div>
      </div>

      {/* Filters Section */}
      <div style={{ backgroundColor: 'white', borderRadius: '8px', border: '1px solid #e2e8f0', marginBottom: '1.5rem' }}>
        <button onClick={() => setShowFilters(!showFilters)} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem', backgroundColor: 'transparent', border: 'none', cursor: 'pointer', borderBottom: showFilters ? '1px solid #e2e8f0' : 'none' }}>
        ...
        {showFilters && (
          <div style={{ padding: '1rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
            <div>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: '600', color: '#64748b', marginBottom: '0.5rem' }}>Start Date</label>
              <input type="date" value={dateRange.start} onChange={(e) => setDateRange({ ...dateRange, start: e.target.value })} style={{ width: '100%', padding: '0.5rem', border: '1px solid #e2e8f0', borderRadius: '6px', fontSize: '0.875rem' }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: '600', color: '#64748b', marginBottom: '0.5rem' }}>End Date</label>
              <input type="date" value={dateRange.end} onChange={(e) => setDateRange({ ...dateRange, end: e.target.value })} style={{ width: '100%', padding: '0.5rem', border: '1px solid #e2e8f0', borderRadius: '6px', fontSize: '0.875rem' }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: '600', color: '#64748b', marginBottom: '0.5rem' }}>Resource</label>
              <select value={selectedResource} onChange={(e) => setSelectedResource(e.target.value)} style={{ width: '100%', padding: '0.5rem', border: '1px solid #e2e8f0', borderRadius: '6px', fontSize: '0.875rem' }}>
                <option value="all">All Resources</option>
                {resources.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
              </select>
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: '600', color: '#64748b', marginBottom: '0.5rem' }}>Milestone</label>
              <select value={selectedMilestone} onChange={(e) => setSelectedMilestone(e.target.value)} style={{ width: '100%', padding: '0.5rem', border: '1px solid #e2e8f0', borderRadius: '6px', fontSize: '0.875rem' }}>
                <option value="all">All Milestones</option>
                {milestones.map(m => <option key={m.id} value={m.id}>{m.reference || m.milestone_ref} - {m.name}</option>)}
              </select>
            </div>
          </div>
        )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
          ...
          <div style={{ padding: '1rem', borderBottom: '1px solid #e2e8f0', backgroundColor: '#f8fafc' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              ...
              <div>
                ...
                <p style={{ margin: 0, fontSize: '0.75rem', color: '#64748b' }}>Overview of milestones, KPIs, and resources</p>
              </div>
            </div>
          </div>
          <div style={{ padding: '1rem' }}>
            <p style={{ fontSize: '0.875rem', color: '#475569', marginBottom: '1rem' }}>Includes project details, milestone status, KPI summary, and resource allocation.</p>
            <button onClick={() => handleGenerateReport('project-summary')} disabled={generating === 'project-summary'} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '0.75rem', backgroundColor: '#2563eb', color: 'white', border: 'none', borderRadius: '6px', cursor: generating === 'project-summary' ? 'not-allowed' : 'pointer', fontWeight: '500', opacity: generating === 'project-summary' ? 0.7 : 1 }}>
              {generating === 'project-summary' ? <RefreshCw size={16} style={{ animation: 'spin 1s linear infinite' }} /> : <Download size={16} />}
        ...
        <div style={{ backgroundColor: 'white', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
          <div style={{ padding: '1rem', borderBottom: '1px solid #e2e8f0', backgroundColor: '#f8fafc' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              <div style={{ width: '40px', height: '40px', borderRadius: '8px', backgroundColor: '#dcfce7', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Clock size={20} style={{ color: '#16a34a' }} />
              </div>
              <div>
                <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: '600' }}>Timesheet Report</h3>
                <p style={{ margin: 0, fontSize: '0.75rem', color: '#64748b' }}>{getFilteredTimesheets().length} entries in selected period</p>
              </div>
            </div>
          </div>
          <div style={{ padding: '1rem' }}>
            <p style={{ fontSize: '0.875rem', color: '#475569', marginBottom: '1rem' }}>Detailed timesheet entries with hours, values, and approval status.</p>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button onClick={() => handleGenerateReport('timesheet')} disabled={generating === 'timesheet'} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '0.75rem', backgroundColor: '#16a34a', color: 'white', border: 'none', borderRadius: '6px', cursor: generating === 'timesheet' ? 'not-allowed' : 'pointer', fontWeight: '500', opacity: generating === 'timesheet' ? 0.7 : 1 }}>
                {generating === 'timesheet' ? <RefreshCw size={16} style={{ animation: 'spin 1s linear infinite' }} /> : <Download size={16} />}PDF
              </button>
              <button onClick={() => handleExportCSV('timesheet')} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '0.75rem', backgroundColor: '#f1f5f9', color: '#475569', border: '1px solid #e2e8f0', borderRadius: '6px', cursor: 'pointer', fontWeight: '500' }}>
          ...
          <div style={{ padding: '1rem', borderBottom: '1px solid #e2e8f0', backgroundColor: '#f8fafc' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              ...
              <div>
                ...
                <p style={{ margin: 0, fontSize: '0.75rem', color: '#64748b' }}>{getFilteredExpenses().length} entries in selected period</p>
              </div>
            </div>
          </div>
          <div style={{ padding: '1rem' }}>
            <p style={{ fontSize: '0.875rem', color: '#475569', marginBottom: '1rem' }}>All expenses with chargeable/non-chargeable breakdown and status.</p>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button onClick={() => handleGenerateReport('expense')} disabled={generating === 'expense'} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '0.75rem', backgroundColor: '#d97706', color: 'white', border: 'none', borderRadius: '6px', cursor: generating === 'expense' ? 'not-allowed' : 'pointer', fontWeight: '500', opacity: generating === 'expense' ? 0.7 : 1 }}>
                {generating === 'expense' ? <RefreshCw size={16} style={{ animation: 'spin 1s linear infinite' }} /> : <Download size={16} />}PDF
              </button>
              <button onClick={() => handleExportCSV('expense')} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '0.75rem', backgroundColor: '#f1f5f9', color: '#475569', border: '1px solid #e2e8f0', borderRadius: '6px', cursor: 'pointer', fontWeight: '500' }}>
                <FileSpreadsheet size={16} />CSV
              </button>
            </div>
          </div>
          ...
          <div style={{ padding: '1rem', borderBottom: '1px solid #e2e8f0', backgroundColor: '#f8fafc' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              ...
              <div>
                ...
                <p style={{ margin: 0, fontSize: '0.75rem', color: '#64748b' }}>{kpis.length} KPIs, {kpiAssessments.length} assessments</p>
              </div>
            </div>
          </div>
          <div style={{ padding: '1rem' }}>
            <p style={{ fontSize: '0.875rem', color: '#475569', marginBottom: '1rem' }}>KPI definitions, targets, and assessment scores summary.</p>
            <button onClick={() => handleGenerateReport('kpi')} disabled={generating === 'kpi'} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '0.75rem', backgroundColor: '#7c3aed', color: 'white', border: 'none', borderRadius: '6px', cursor: generating === 'kpi' ? 'not-allowed' : 'pointer', fontWeight: '500', opacity: generating === 'kpi' ? 0.7 : 1 }}>
              {generating === 'kpi' ? <RefreshCw size={16} style={{ animation: 'spin 1s linear infinite' }} /> : <Download size={16} />}
      ...
      {/* Invoicing Section */}
      {(canGenerateCustomerInvoice(userRole) || canGenerateThirdPartyInvoice(userRole)) && (
        <div style={{ backgroundColor: 'white', borderRadius: '8px', border: '1px solid #e2e8f0', marginBottom: '1.5rem' }}>
          <button onClick={() => setShowInvoiceSection(!showInvoiceSection)} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem', backgroundColor: '#fef3c7', border: 'none', cursor: 'pointer', borderRadius: showInvoiceSection ? '8px 8px 0 0' : '8px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <DollarSign size={20} style={{ color: '#d97706' }} />
              <span style={{ fontWeight: '600', color: '#92400e', fontSize: '1rem' }}>Invoice Generation</span>
              <span style={{ backgroundColor: '#fbbf24', color: '#78350f', padding: '0.125rem 0.5rem', borderRadius: '4px', fontSize: '0.75rem', fontWeight: '500' }}>
                {(canGenerateCustomerInvoice(userRole) ? 1 : 0) + (canGenerateThirdPartyInvoice(userRole) ? 1 : 0)} types available
              </span>
            </div>
            {showInvoiceSection ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
          </button>
          ...
          {showInvoiceSection && (
            <div style={{ padding: '1.5rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '1.5rem' }}>
                {/* Customer Invoice */}
                {canGenerateCustomerInvoice(userRole) && (
                  <div style={{ padding: '1.5rem', backgroundColor: '#f0fdf4', borderRadius: '8px', border: '1px solid #86efac' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
                      ...
                      <div>
                        <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: '600', color: '#166534' }}>Customer Invoice</h3>
                        <p style={{ margin: 0, fontSize: '0.75rem', color: '#15803d' }}>Invoice Government of Jersey for completed milestones</p>
                      </div>
                    </div>
                    <div style={{ marginBottom: '1rem' }}>
                      <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: '600', color: '#15803d', marginBottom: '0.5rem' }}>Select Milestone *</label>
                      <select value={invoiceMilestone} onChange={(e) => setInvoiceMilestone(e.target.value)} style={{ width: '100%', padding: '0.75rem', border: '1px solid #86efac', borderRadius: '6px', fontSize: '0.875rem', backgroundColor: 'white' }}>
                        <option value="">Choose a milestone...</option>
                        {getCompletedMilestones().length > 0 ? (
                          getCompletedMilestones().map(m => <option key={m.id} value={m.id}>{m.reference || m.milestone_ref} - {m.name} (Completed)</option>)
                        ) : (
                          milestones.map(m => <option key={m.id} value={m.id}>{m.reference || m.milestone_ref} - {m.name} ({m.status})</option>)
                        )}
                      </select>
                      {getCompletedMilestones().length === 0 && <p style={{ fontSize: '0.75rem', color: '#f59e0b', marginTop: '0.25rem' }}>No completed milestones. Showing all milestones.</p>}
                    </div>
                    <p style={{ fontSize: '0.8rem', color: '#166534', marginBottom: '1rem' }}><strong>Includes:</strong> Deliverables, approved timesheets, and chargeable expenses for the selected milestone.</p>
                    <button onClick={handleGenerateCustomerInvoice} disabled={generating === 'customer-invoice' || !invoiceMilestone} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '0.75rem', backgroundColor: !invoiceMilestone ? '#9ca3af' : '#16a34a', color: 'white', border: 'none', borderRadius: '6px', cursor: !invoiceMilestone || generating === 'customer-invoice' ? 'not-allowed' : 'pointer', fontWeight: '600', opacity: generating === 'customer-invoice' ? 0.7 : 1 }}>
                      {generating === 'customer-invoice' ? <RefreshCw size={16} style={{ animation: 'spin 1s linear infinite' }} /> : <Download size={16} />}
                      {generating === 'customer-invoice' ? 'Generating...' : 'Generate Customer Invoice'}
                    </button>
                  </div>
                )}

                {/* Third-Party Partner Invoice */}
                {canGenerateThirdPartyInvoice(userRole) && (
                  <div style={{ padding: '1.5rem', backgroundColor: '#fef3c7', borderRadius: '8px', border: '1px solid #fcd34d' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
                      <UserCheck size={24} style={{ color: '#d97706' }} />
                      <div>
                        <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: '600', color: '#92400e' }}>Third-Party Partner Invoice</h3>
                        <p style={{ margin: 0, fontSize: '0.75rem', color: '#a16207' }}>Generate invoice summary for partner payments</p>
                      </div>
                    </div>
                    <div style={{ marginBottom: '1rem' }}>
                      <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: '600', color: '#92400e', marginBottom: '0.5rem' }}>Select Partner *</label>
                      <select value={invoicePartner} onChange={(e) => setInvoicePartner(e.target.value)} style={{ width: '100%', padding: '0.75rem', border: '1px solid #fcd34d', borderRadius: '6px', fontSize: '0.875rem', backgroundColor: 'white' }}>
                        <option value="">Choose a third-party partner...</option>
                        {getThirdPartyPartners().length > 0 ? (
                          getThirdPartyPartners().map(p => <option key={p.id} value={p.id}>{p.name} - {p.role} ({formatCurrency(p.cost_price || p.daily_rate)}/day)</option>)
                        ) : (
                          <option value="" disabled>No third-party partners found</option>
                        )}
                      </select>
                      {getThirdPartyPartners().length === 0 && <p style={{ fontSize: '0.75rem', color: '#dc2626', marginTop: '0.25rem' }}>No third-party resources configured. Add them in Resources page.</p>}
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '1rem' }}>
                      <div>
                        <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: '600', color: '#92400e', marginBottom: '0.5rem' }}>From Date</label>
                        <input type="date" value={invoiceDateRange.start} onChange={(e) => setInvoiceDateRange({ ...invoiceDateRange, start: e.target.value })} style={{ width: '100%', padding: '0.5rem', border: '1px solid #fcd34d', borderRadius: '6px', fontSize: '0.875rem' }} />
                      </div>
                      <div>
                        <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: '600', color: '#92400e', marginBottom: '0.5rem' }}>To Date</label>
                        <input type="date" value={invoiceDateRange.end} onChange={(e) => setInvoiceDateRange({ ...invoiceDateRange, end: e.target.value })} style={{ width: '100%', padding: '0.5rem', border: '1px solid #fcd34d', borderRadius: '6px', fontSize: '0.875rem' }} />
                      </div>
                    </div>
                    <p style={{ fontSize: '0.8rem', color: '#92400e', marginBottom: '1rem' }}><strong>Includes:</strong> Time worked (at cost price) and expenses for the selected partner and period.</p>
                    <button onClick={handleGeneratePartnerInvoice} disabled={generating === 'partner-invoice' || !invoicePartner} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '0.75rem', backgroundColor: !invoicePartner ? '#9ca3af' : '#d97706', color: 'white', border: 'none', borderRadius: '6px', cursor: !invoicePartner || generating === 'partner-invoice' ? 'not-allowed' : 'pointer', fontWeight: '600', opacity: generating === 'partner-invoice' ? 0.7 : 1 }}>
                      {generating === 'partner-invoice' ? <RefreshCw size={16} style={{ animation: 'spin 1s linear infinite' }} /> : <Download size={16} />}
                      {generating === 'partner-invoice' ? 'Generating...' : 'Generate Partner Invoice'}
                    </button>
                  </div>
                )}
              </div>
      ...
      {/* Info Box */}
      <div style={{ backgroundColor: '#eff6ff', borderLeft: '4px solid #3b82f6', padding: '1rem', borderRadius: '0 8px 8px 0' }}>
        <h4 style={{ margin: '0 0 0.5rem', color: '#1e40af', fontSize: '0.875rem' }}>Report & Invoice Tips</h4>
        <ul style={{ margin: 0, paddingLeft: '1.25rem', fontSize: '0.875rem', color: '#1e40af' }}>
          ...
          <li><strong>Customer Invoice:</strong> Select a completed milestone to generate an invoice for Government of Jersey</li>
          <li><strong>Partner Invoice:</strong> Select a third-party resource and date range to summarise their work at cost price</li>
        </ul>
      </div>
