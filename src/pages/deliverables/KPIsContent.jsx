/**
 * KPIs Content - Tab content for DeliverablesHub
 * 
...
export default function KPIsContent() {
  ...
  useEffect(() => {
    if (projectId) fetchKPIs(projectId);
  }, [projectId]);
  ...
  useEffect(() => {
    if (showAddForm && kpis.length > 0) {
      ...
      setNewKPI(prev => ({ ...prev, kpi_ref: `KPI${String(maxRef + 1).padStart(2, '0')}` }));
    } else if (showAddForm) {
  ...
  async function fetchKPIs(projId) {
    ...
    if (!pid) return;
    try {
      const data = await kpisService.getAll(pid, { orderBy: { column: 'kpi_ref', ascending: true } });
      setKpis(data || []);
  ...
  async function handleRefresh() { setRefreshing(true); await fetchKPIs(); }

  async function handleAddKPI() {
    if (!newKPI.kpi_ref || !newKPI.name) { alert('Please fill in KPI Reference and Name'); return; }
    if (kpis.some(k => k.kpi_ref.toLowerCase() === newKPI.kpi_ref.toLowerCase())) {
      alert(`KPI "${newKPI.kpi_ref}" already exists.`); return;
    }
    setSaving(true);
    try {
      await kpisService.create({
        project_id: projectId, kpi_ref: newKPI.kpi_ref.toUpperCase(), name: newKPI.name,
        category: newKPI.category, target: parseInt(newKPI.target) || 90, unit: '%',
        description: newKPI.description || null, measurement_method: newKPI.measurement_method || null,
        frequency: newKPI.frequency || 'Monthly', data_source: newKPI.data_source || null,
        calculation: newKPI.calculation || null, remediation: newKPI.remediation || null,
        current_value: 0, created_by: currentUserId
      });
      await fetchKPIs();
      setShowAddForm(false);
      setNewKPI({ kpi_ref: '', name: '', category: 'Time Performance', target: 90, description: '', measurement_method: '', frequency: 'Monthly', data_source: '', calculation: '', remediation: '' });
    } catch (error) { alert('Failed to add KPI: ' + error.message); }
  ...
  async function handleConfirmDelete() {
    ...
    try {
      ...
      const { data: linkedDeliverables } = await supabase.from('deliverable_kpis').select('id').eq('kpi_id', kpi.id);
    ...
    } catch (error) { alert('Failed to delete KPI: ' + error.message); }
    finally { setSaving(false); }
  }
  ...
  function getKPIStatus(kpi) {
    ...
    if (!assessments || assessments.total === 0) return { label: 'Not Started', class: 'not-started', icon: Clock };
    ...
    <div className="kpis-page" data-testid="kpis-page">
      ...
      <div className="kpi-content" data-testid="kpis-content">
                ...
                {kpis.map(kpi => {
                  ...
                  return (
                    <tr key={kpi.id} onClick={() => navigate(`/kpis/${kpi.id}`)} data-testid={`kpi-row-${kpi.id}`}>
                      ...
                      <td><span className={`kpi-current ${status.class}`}>{currentScore}%</span></td>
                      <td>{assessments ? <span className="kpi-assessments">{assessments.met}/{assessments.total} passed</span> : <span className="kpi-assessments-none">None yet</span>}</td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        <div className="kpi-info-box">
          <div className="kpi-info-header"><Info size={18} /> How KPI Scores Work</div>
      ...
      <ConfirmDialog isOpen={deleteDialog.isOpen} onClose={() => setDeleteDialog({ isOpen: false, kpi: null })} onConfirm={handleConfirmDelete} title="Delete KPI?" message={deleteDialog.kpi ? `This will permanently delete "${deleteDialog.kpi.kpi_ref}: ${deleteDialog.kpi.name}".` : ''} confirmText="Delete KPI" cancelText="Cancel" type="danger" isLoading={saving} />
    </div>
  );
}
